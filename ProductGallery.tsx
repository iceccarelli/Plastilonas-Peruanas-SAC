...
/**
 * ProductGallery — data-driven gallery for the product detail page.
 *
 *  - Reads `product.gallery` (falls back to `[product.image]`, then to the
 *    branded <ProductVisual> placard if there is no image at all — so every
 *    one of the 34 products degrades gracefully).
 *  - Single-image products render ONLY the main image (no empty thumbnail strip).
 *  - Multi-image products get a thumbnail strip + full-screen lightbox with
 *    keyboard navigation. This is the behaviour a mining / construction buyer
 *    uses to vet a supplier before requesting a quote.
 *  - Main image keeps the Ken Burns treatment and carries `priority` (it is the
 *    LCP element on the detail page).
 */
...
export default function ProductGallery({ product }: { product: Product }) {
  ...
  // No image at all -> brand placard, identical to the previous fallback.
  if (images.length === 0) {
    return (
  ...
  return (
    <div>
      ...
      {/* Thumbnail strip (only when there is more than one real photo) */}
      {hasMultiple && (
        <div
