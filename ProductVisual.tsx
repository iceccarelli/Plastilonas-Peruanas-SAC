 ...
 * por lo que el catálogo muestra una "placa" de categoría deliberada y
 * consistente (gradiente navy + textura de puntos del hero + ícono industrial +
 * etiqueta) en SVG/CSS puro: cero peso de red, seguro en conexiones lentas del
 * Perú, y evidentemente un diseño intencional, NO una foto falsa.
 *
 * El mapa `MAP` resuelve el ícono/acento a partir de `product.category`, que
 * ahora corresponde a las 11 FAMILIAS del catálogo. Cada familia tiene un ícono
 * y un tinte propios para facilitar el escaneo tipo grid.
 *
...
interface CategoryVisual {
  icon: LucideIcon;
  /** Tinte de acento para diferenciar familias de un vistazo. */
  accent: string;
}

const FALLBACK: CategoryVisual = { icon: Layers, accent: '#059669' };

// Se hace match por un token distintivo del nombre de cada familia, de modo que
// el orden no importa y no hay colisiones entre familias.
