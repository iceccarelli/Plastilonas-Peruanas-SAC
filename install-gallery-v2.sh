#!/usr/bin/env bash
# ============================================================================
# install-gallery-v2.sh — Galería estilo AWS (multi-tarjeta + swipe Tinder +
# Ken Burns + autoplay 3s/5s con pausa al hover). Usa los DOS sets v2
# fotorrealistas. Idempotente. No toca git.
#   - Coloca 144 imágenes nuevas (sufijos c/d = set1, e/f = set2) junto a las
#     existentes (a/b).
#   - Reescribe lib/machinery.ts (multi-vista) y components/MachineryGallery.tsx.
#   - El cableado de app/page.tsx NO cambia (ya inserta <MachineryGallery/>).
# ============================================================================
set -euo pipefail
fail(){ echo "MISS: $1" >&2; exit 1; }
[ -f app/page.tsx ] || fail "ejecútelo desde la raíz del repo"
grep -q "MachineryGallery" app/page.tsx || fail "app/page.tsx no cablea <MachineryGallery/> (instale primero la galería base)"
...
S1="$TMP/s1/galeria-maquinaria-v2/img"; S2="$TMP/s2/galeria-maquinaria-v2/img"
[ -d "$S1" ] || S1="$(dirname "$(find "$TMP/s1" -name '01-extrusion-rafia-a.webp' | head -1)")"
[ -d "$S2" ] || S2="$(dirname "$(find "$TMP/s2" -name '01-extrusion-rafia-a.webp' | head -1)")"
...
cpv(){ # $1 src dir, $2 slug, $3 srcvar, $4 dstvar
  for ext in webp png; do [ -f "$1/$2-$3.$ext" ] && cp -f "$1/$2-$3.$ext" "$DEST/$2-$4.$ext"; done
  [ -f "$1/$2-$3-thumb.webp" ] && cp -f "$1/$2-$3-thumb.webp" "$DEST/$2-$4-thumb.webp"; :
}
n=0
for s in "${slugs[@]}"; do
  cpv "$S1" "$s" a c; cpv "$S1" "$s" b d
  cpv "$S2" "$s" a e; cpv "$S2" "$s" b f
  n=$((n+1))
done
echo "ok   imágenes v2 colocadas ($(ls -1 "$DEST" | wc -l | tr -d ' ') archivos en $DEST)"

cat > lib/machinery.ts <<'PLASTILONAS_EOF_9c3f'
/**
 * Galería ILUSTRATIVA del proceso de fabricación — Plastilonas Peruanas SAC.
 *
 * Imágenes EDUCATIVAS de las categorías de maquinaria del sector (Big Bags,
 * geomembranas, carpas, mallas, lonas). NO son fotografías de la planta de
 * Plastilonas. La UI las rotula siempre como "galería ilustrativa".
 *
 * Cada máquina tiene VARIAS vistas fotorrealistas (dos lotes de render).
 * El carrusel rota las vistas de una misma máquina cada ~3 s (Ken Burns) y
 * avanza de máquina en máquina cada ~5 s; el usuario puede deslizar cuando
 * quiera. Data-driven igual que lib/products.ts.
 */
...
export interface MachineryItem {
  ...
  linea: string;
  fuente: string;
  /** 4-6 vistas fotorrealistas de la misma máquina (rotan con Ken Burns). */
  views: MachineryView[];
}
...
export const machinery: MachineryItem[] = [
  {
    ...
    views: [
      { webp: `${BASE}/01-extrusion-rafia-a.webp`, thumb: `${BASE}/01-extrusion-rafia-a-thumb.webp` },
      { webp: `${BASE}/01-extrusion-rafia-b.webp`, thumb: `${BASE}/01-extrusion-rafia-b-thumb.webp` },
  ...
  },
  {
    ...
    views: [
      { webp: `${BASE}/02-telar-circular-a.webp`, thumb: `${BASE}/02-telar-circular-a-thumb.webp` },
  ...
  },
  {
    ...
    views: [
      { webp: `${BASE}/03-laminado-recubrimiento-a.webp`, thumb: `${BASE}/03-laminado-recubrimiento-a-thumb.webp` },
      { webp: `${BASE}/03-laminado-recubrimiento-b.webp`, thumb: `${BASE}/03-laminado-recubrimiento-b-thumb.webp` },
  ...
  },
  {
    ...
    views: [
      { webp: `${BASE}/04-impresora-flexografica-a.webp`, thumb: `${BASE}/04-impresora-flexografica-a-thumb.webp` },
      { webp: `${BASE}/04-impresora-flexografica-b.webp`, thumb: `${BASE}/04-impresora-flexografica-b-thumb.webp` },
  ...
  },
  {
    ...
    views: [
      { webp: `${BASE}/05-corte-automatico-a.webp`, thumb: `${BASE}/05-corte-automatico-a-thumb.webp` },
      { webp: `${BASE}/05-corte-automatico-b.webp`, thumb: `${BASE}/05-corte-automatico-b-thumb.webp` },
  ...
  },
  {
    ...
    views: [
      { webp: `${BASE}/07-calandra-pvc-a.webp`, thumb: `${BASE}/07-calandra-pvc-a-thumb.webp` },
      { webp: `${BASE}/07-calandra-pvc-b.webp`, thumb: `${BASE}/07-calandra-pvc-b-thumb.webp` },
  ...
  },
  {
    ...
    views: [
      { webp: `${BASE}/08-soldadora-alta-frecuencia-a.webp`, thumb: `${BASE}/08-soldadora-alta-frecuencia-a-thumb.webp` },
      { webp: `${BASE}/08-soldadora-alta-frecuencia-b.webp`, thumb: `${BASE}/08-soldadora-alta-frecuencia-b-thumb.webp` },
  ...
  },
  {
    ...
    views: [
      { webp: `${BASE}/09-soldadora-cuna-caliente-a.webp`, thumb: `${BASE}/09-soldadora-cuna-caliente-a-thumb.webp` },
      { webp: `${BASE}/09-soldadora-cuna-caliente-b.webp`, thumb: `${BASE}/09-soldadora-cuna-caliente-b-thumb.webp` },
  ...
  },
  {
    ...
    views: [
      { webp: `${BASE}/10-soldadora-aire-caliente-a.webp`, thumb: `${BASE}/10-soldadora-aire-caliente-a-thumb.webp` },
      { webp: `${BASE}/10-soldadora-aire-caliente-b.webp`, thumb: `${BASE}/10-soldadora-aire-caliente-b-thumb.webp` },
  ...
  },
  {
    ...
    views: [
      { webp: `${BASE}/11-curvadora-tubo-a.webp`, thumb: `${BASE}/11-curvadora-tubo-a-thumb.webp` },
      { webp: `${BASE}/11-curvadora-tubo-b.webp`, thumb: `${BASE}/11-curvadora-tubo-b-thumb.webp` },
  ...
  },
  {
    ...
    views: [
      { webp: `${BASE}/12-telar-raschel-a.webp`, thumb: `${BASE}/12-telar-raschel-a-thumb.webp` },
      { webp: `${BASE}/12-telar-raschel-b.webp`, thumb: `${BASE}/12-telar-raschel-b-thumb.webp` },
...
echo "ok   lib/machinery.ts (multi-vista)"
...
echo "            git commit -m \"feat(gallery): carrusel estilo AWS + swipe + KenBurns + sets v2\""
