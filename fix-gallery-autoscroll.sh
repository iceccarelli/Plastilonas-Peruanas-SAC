#!/usr/bin/env bash
# fix-gallery-autoscroll.sh — Corrige el auto-desplazamiento a la galería al
# cargar la página. Solo reescribe components/MachineryGallery.tsx.
# El filmstrip ya no arrastra la vista vertical; no centra en el montaje.
set -euo pipefail
[ -f components/MachineryGallery.tsx ] || { echo "MISS: no encuentro components/MachineryGallery.tsx"; exit 1; }
...
echo "ok   components/MachineryGallery.tsx actualizado (sin auto-scroll al cargar)"
echo "Verifique:  rm -rf .next && npm run build"
echo "Commit:     git add components/MachineryGallery.tsx && git commit -m \"fix(gallery): evitar auto-scroll a la galería al cargar\" && git push origin main"
